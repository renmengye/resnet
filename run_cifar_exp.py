...
from resnet.data import CIFAR10Dataset, CIFAR100Dataset
from resnet.expr import (BasicEnvironment, gen_id, get_device,
                         get_environ_from_json)
...
def get_environ():
  ...
  if FLAGS.env is not None:
    return get_environ_from_json(FLAGS.env)

  if FLAGS.data_folder is None:
    data_folder = os.path.join("data", FLAGS.dataset)
  else:
